...
let LoggingService = class LoggingService {
    getChangedValues(oldData, newData) {
        const changes = {};
        ...
        for (const key of allKeys){
            ...
            if (Array.isArray(oldValue) || Array.isArray(newValue)) {
                ...
                const oldArrayStr = JSON.stringify([
                    ...oldArray
                ].sort());
                const newArrayStr = JSON.stringify([
                    ...newArray
                ].sort());
    ...
    async logDatabaseAction(table, action, recordId, details, operationDetails, userId) {
        try {
            let logDetails;
            ...
            if (logDetails && Object.keys(logDetails).length > 0) {
                ...
                const formattedDetails = Object.entries(logDetails).map(([key, value])=>{
                    // Format field names
                    ...
                    switch(key){
                        ...
                        default:
                            ...
                            formattedKey = key.split('_').map((word)=>word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
                    }
    ...
    async trackUserAction(userId, sessionId, pageVisited, actionType, actionData, source) {
        try {
            ...
            this.logger.debug(`Tracked action: ${actionType} by user ${userId} on page ${pageVisited} from source ${source || this.APP_SOURCE}`);
        } catch (error) {
            this.logger.error(`Error tracking user action: ${error.message}`, error.stack);
        // Don't rethrow - analytics should never break the application
        }
    }
    async createUserSession(userId, req, sessionType, source) {
        try {
    ...
    constructor(prisma){
        this.prisma = prisma;
        this.logger = new _common.Logger(LoggingService.name);
        this.APP_SOURCE = 'main-app';
    }
};
